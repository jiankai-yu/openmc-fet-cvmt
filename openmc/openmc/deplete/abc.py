...
class ReactionRateHelper(ABC):
    ...
    def __init__(self, n_nucs, n_react, fet_deplete=None): #FETs 
        #
        self._nuclides = None # store non-fet nucs 
        self._rate_tally = None # store non-fet tally
        self._results_cache = empty((n_nucs, n_react)) # non-fet tally results 
        #
        if fet_deplete is not None:
            ...
            if fet_deplete['name']== 'zernike':
                ...
                mp = order//2 + 1 
                  ...
                  fet_deplete['name']=='legendre-z'):
                mp = order + 1
            #
            n_nucs_fet = len(fet_deplete['nuclides_fet'])
            self._nuclides_fet = None # store fet nucs
            self._rate_tally_fet = None # store fet tally
            self._results_cache_fet = empty((n_nucs_fet, n_react * mp)) # fet tally results   
            self._results_cache_final = empty((n_nucs, n_react * mp)) # total results       
    ...
    @property
    def nuclides_fet(self):
        """List of nuclides with requested reaction rates"""
        return self._nuclides_fet
    
    @nuclides_fet.setter
    def nuclides_fet(self, nuclides_fet):
        check_type("nuclides_fet", nuclides_fet, list, str)
        self._nuclides_fet = nuclides_fet
        for i, _ in enumerate(self._rate_tally_fet):
            self._rate_tally_fet[i].nuclides = nuclides_fet
    ...
    def divide_by_adens(self, number, fet_deplete=None):
        ...
        if fet_deplete is None:
            ...
            #
            #print(len(number_fet))   
            #print(self._results_cache_final.shape[0])            
            mask = nonzero(number_fet)
            results = self._results_cache_final 
            for col in range(results.shape[1] // mp): #FETs 
                for i in range(mp):
                    results[mask, col * mp + i] /= number_fet[mask]            
            return results
        
    def divide_by_adens_fet(self, number, fet_deplete):
        """Normalize reaction rates by number of nuclides
        ...
        #       
        mask = nonzero(number_fet)
        results = self._results_cache_fet
        for col in range(results.shape[1] // mp): #FETs 
            for i in range(mp):
                results[mask, col * mp + i] /= number_fet[mask]            
        return results
...
class Integrator(ABC):
    ...
    def _export_to_pdf(self, stage):
        ...
        for i, mat in enumerate(number.materials):
            fet_type = fet_deplete['name']
            if 'orders_map' in fet_deplete.keys():
                order = fet_deplete['orders_map'][int(mat)]
            else:
                order = fet_deplete['order']
            if fet_type == 'zernike':
            ...
            if i in print_fuel:
                for nuc in number.nuclides:
                    if nuc in print_nuc:
                        coeff = number.get_atom_density(mat, nuc, mp=mp_mat)

...
from openmc.lib import (
    ...
    ZernikeFilter, ZernikeRadialFilter, SpatialLegendreFilter)
from .abc import (
...
class DirectReactionRateHelper(ReactionRateHelper):
    ...
    def generate_tallies(self, materials, scores, fet_deplete=None):
        ...
        if fet_deplete is not None:
            ...
            self._rate_tally_fet = [Tally() for mat in materials]
            for i, mat in enumerate(materials):
                self._rate_tally_fet[i].writable = True
                self._rate_tally_fet[i].scores = scores
                self._rate_tally_fet[i].estimator = 'collision'
                self._rate_tally_fet[i].filters = [MaterialFilter([mat])]
                self._rate_tally_fet[i].nuclides = fet_deplete['nuclides_fet']
                #
                temp_loc = {}
                if fet_deplete['name'] == 'zernike':
                    offset = fet_deplete['offset'][mat.id]
                    order_mat = fet_deplete['orders_map'][mat.id]
                    temp_loc['x'], temp_loc['y'] =  offset[0], offset[1]
                    #print(offset)
                    temp_loc['r'] = fet_deplete['radius']
                    zer_filter = ZernikeFilter()
                    zer_filter.order = order_mat
                    zer_filter.params = temp_loc
                    self._rate_tally_fet[i].filters += [zer_filter]
                    ...
                    zer1d_filter.order = order_mat
                    zer1d_filter.params = temp_loc
                    self._rate_tally_fet[i].filters += [zer1d_filter]
                elif (fet_deplete['name']=='legendre-x'):
                    offset = fet_deplete['offset'][mat.id]
                    order_mat = fet_deplete['orders_map'][mat.id]
                    temp_loc['minimum'], temp_loc['maximum'] = offset[0], offset[1]
                    temp_loc['axis'] = 'x'
                    legx_filter = SpatialLegendreFilter()
                    legx_filter.order = order_mat
                    legx_filter.params = temp_loc
                    self._rate_tally_fet[i].filters += [legx_filter]                 
                elif (fet_deplete['name']=='legendre-y'): 
                    offset = fet_deplete['offset'][mat.id]
                    order_mat = fet_deplete['orders_map'][mat.id]
                    temp_loc['minimum'], temp_loc['maximum'] = offset[0], offset[1]
                    temp_loc['axis'] = 'y'
                    legy_filter = SpatialLegendreFilter()
                    legy_filter.order = order_mat
                    legy_filter.params = temp_loc
                    self._rate_tally_fet[i].filters += [legy_filter] 
                elif (fet_deplete['name']=='legendre-z'):
                    offset = fet_deplete['offset'][mat.id]
                    order_mat = fet_deplete['orders_map'][mat.id]
                    temp_loc['minimum'], temp_loc['maximum'] = offset[0], offset[1]
                    temp_loc['axis'] = 'z'
                    legz_filter = SpatialLegendreFilter()
                    legz_filter.order = order_mat
                    legz_filter.params = temp_loc
                    self._rate_tally_fet[i].filters += [legz_filter]                 
    ...
    def get_material_rates(self, mat_id, nuc_index, react_index, fet_deplete=None):
        ...
        full_tally_res = self._rate_tally.results[mat_id, :, 1] # FETs 
        ...
        #FETs  
        #print('nuc_index=', nuc_index) 
        if fet_deplete is not None:
            mp_max = 1 
            mp_mat = 1
            legendre = False 
            order_max = fet_deplete['order']
            keys = list(fet_deplete['orders_map'].keys())
            order = fet_deplete['orders_map'][keys[mat_id]]
            keys = list(fet_deplete['nucs_from_mat_list'].keys())
            nucs_fet_mat = fet_deplete['nucs_from_mat_list'][keys[mat_id]]
            if fet_deplete['name']== 'zernike':
                mp_mat = (order+1)*(order+2)//2
                mp_max = (order_max+1)*(order_max+2)//2
            elif fet_deplete['name']=='zernike1d':
                mp_mat = (order)//2 + 1 
                mp_max = (order_max)//2 + 1 
            elif (fet_deplete['name']=='legendre-x' 
                  or fet_deplete['name']=='legendre-y' 
                  or fet_deplete['name']=='legendre-z'):
                mp_mat = order + 1
                mp_max = order_max + 1
            self._results_cache_fet.fill(0.0)
            #print(self._rate_tally[mat_id].results)
            nuc_index_fet = [i for i in range(len(fet_deplete['nuclides_fet']))]
            #print('nuc_index_fet=', nuc_index_fet) 
            for i in range(mp_mat):
                #print(self._rate_tally_fet.results[i * mat_id, :, 1])
                #full_tally_res = self._rate_tally_fet.results[mp * mat_id + i, :, 1] 
                factor = zer.normalize(fet_deplete['name'], fet_deplete['order'], i)
                full_tally_res_fet = self._rate_tally_fet[mat_id].results[i, :, 1]
                #print('shape=', np.shape(self._rate_tally_fet[mat_id].results))                
                for i_tally, (i_nuc, i_react) in enumerate(
                    product(nuc_index_fet, react_index)):
                    self._results_cache_fet[i_nuc, i_react * mp_max + i] = full_tally_res_fet[i_tally] * factor
            #print(self._results_cache) # testing for FETs
            
            # combine self._results_cache and self._results_cache_fet
            index_nucs_fet = [self._nuclides_fet.index(i) for i in nucs_fet_mat]
            index_nucs_mat = [self._nuclides.index(i) for i in nucs_fet_mat if i in self._nuclides]
            self._results_cache_final.fill(0.0)
            for i_nuc in range(len(nuc_index)):
                for i_react in range(len(react_index)):
                    self._results_cache_final[i_nuc,i_react * mp_max] = self._results_cache[i_nuc, i_react]
            #print(index_nucs_mat)
            for i_nuc in range(len(index_nucs_mat)):
                for i_react in range(len(react_index)):
                    for i in range(mp_mat):
                        self._results_cache_final[index_nucs_mat[i_nuc], i_react * mp_max + i] = \
                            self._results_cache_fet[index_nucs_fet[i_nuc], i_react * mp_max + i]
                    #print(self._results_cache_fet[i_nuc, i_react * mp], self._results_cache[index_nucs_mat[i_nuc], i_react])
            return self._results_cache_final
        else:
            return self._results_cache

...
class Settings(object):
    ...
    def fet_deplete(self, fet):
        ...
        keys = ('enable', 'order', 'radius', 'name', 'offset', 'print', 
                'print_fuel', 'print_nuclide', 'cvmt', 'axial_filter', 'orders_map')
        for key, value in fet.items():
            ...
            if key == 'enable':
                ...
                name = 'fet deplete name'
                cv.check_type(name, value, str)
            elif key == 'offset':
                name = 'fet deplete offset'
                cv.check_type(name, value, Iterable)
            elif key == 'print':
                name = 'fet deplete print'
                ...
                cv.check_type(name, value, bool)
            elif key == 'axial_filter':
                name = 'fet deplete axial filter'
                cv.check_type(name, value, Iterable)
            elif key == 'orders_map':
                name = 'fet deplete orders map'
                cv.check_type(name, value, Iterable)
        self._fet_deplete = fet
    
    # cvmt
    @cvmt_intervals.setter     
    def cvmt_intervals(self, cvmt_intervals):
        cv.check_type('cvmt number of intervals', cvmt_intervals, Integral)
        self._cvmt_intervals = cvmt_intervals
    ...
    def export_to_xml(self, path='settings.xml'):
        ...
        # cvmt 
        self._create_cvmt_intervals_subelement(root_element)
    ...
    def from_xml(cls, path='settings.xml'):
        ...
        # cvmt 
        settings._cvmt_intervals_subelement(root_element)

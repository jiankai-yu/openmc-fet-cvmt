...
Particle::sampling_cvmt(Particle *p, double d_boundary){
  ...
  //
  // p->E_ = 1e-5;
  // p->coord_[p->n_coord_-1].r = {0.0, 0.0, 0.0};
  // d_boundary = 0.1;
  // 
  xs_t.resize(settings::cvmt_intervals + 1);
  simpsons_path_integration(optical_depth, d_boundary, xs_t, false, 0);
  //std::cout<<optical_depth<<std::endl;
  ...
  if(prn() <= PNC) {   //! no collision
    ...
    xyz_orig = p->coord_[p->n_coord_-1].r;
    //! cross sections are updated for later tallying of keff and user tallies
    move_particle_coord(p->coord_[p->n_coord_-1], d_collision);
    model::materials[material_]->calculate_xs(*p);
    //! move particle back
    p -> coord_[p -> n_coord_-1].r  = xyz_orig;
  }
  // std::cout<<d_collision<<std::endl;
  //std::cout<<d_boundary<<'\t'<<optical_depth<<'\t'<<PNC<<'\t'<<d_collision<<'\t'<<p->E_<<std::endl;
  //std::exit(-1);
  return d_collision;
}
...
Particle::simpsons_path_integration(double &optical_depth, double distance, std::vector<double> &xs_t, bool dbg_file, int it_num){
  ...
  //
  ds = distance / (double)(settings::cvmt_intervals);
  // 
  if (dbg_file) {
       ...
       fprintf(fp, "%15.7f %15.7f %15.7f %15.7f %15.7f \n",  
            this -> coord_[this -> n_coord_-1].u[0], 
            this -> coord_[this -> n_coord_-1].u[1], 
            this -> coord_[this -> n_coord_-1].u[2], 
            this -> E_, ds);
  }

  for(i=1; i<=(settings::cvmt_intervals+1); i++){ 
    ...
    if(dbg_file) {
       ...
       printf("%15.7f %15.7f %15.7f %15.7f %15.7f \n", 
              this -> coord_[this -> n_coord_-1].r[0],  
              this -> coord_[this -> n_coord_-1].r[1], 
              this -> coord_[this -> n_coord_-1].r[2], 
              new_temp, xs_t[i-1]);
       fprintf(fp, "%15.7f %15.7f %15.7f %15.7f %15.7f \n", 
              this -> coord_[this -> n_coord_-1].r[0],  
              this -> coord_[this -> n_coord_-1].r[1], 
  ...
  //
  optical_depth = 0.0; 
  for( i=1; i<=(settings::cvmt_intervals-1); i=i+2){
     //! Add to the integral
...
Particle::estimate_flight_distance(std::vector<double> xs_t, double distance, double tau_hat, double &s){
  ...
  while(optical_depth <= tau_hat){
    if (index > (settings::cvmt_intervals - 1)) {
      tau_overrun = true;
  ...
  if (tau_overrun) {
    ...
    if (fabs(a) > 1E-10) {
      ...
      if (s < (ds * (index-1)) || s > (ds * (index+1)) ) {
        ...
        printf("xs_t(3) = %15.7f\n", xs_t[index+1]);
        printf("s = %15.7f\n", s);
        printf("distance = %15.7f\n", distance);
        printf("optical_depth = %15.7f\n", optical_depth);
        printf("tau_hat = %15.7f\n", tau_hat);
        printf("delta_tau_hat = %15.7f\n", delta_tau_hat);
        printf("index = %15d\n", index);
        printf("ds*(index-1) = %15.7f\n", ds * (index-1));
        printf("ds*(index) = %15.7f\n", ds * (index));
        fatal_error("The s for the constant cross section case lies out of bounds");
      }
    }
  }
  if (s > distance || s < 0.0) {

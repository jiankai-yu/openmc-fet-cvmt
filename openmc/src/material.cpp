    ...
    for (auto node_nuc : node.children("nuclide")) {
      ...
      if (units == "macro") {
        ...
        if(check_for_node(node_nuc, "poly_coeffs")){
		  ...
		  auto order = 0;
		  // format of offset for zernike or zernike1d is not determined. 
          if(type == "zernike1d") {
            order = 2 * (n_coeffs - 2);
		  ...
		  //
		  // for (int kk=0; kk<n_coeffs+2; ++kk){
		  //   std::cout<<coeffs[kk]<<"\t";
		  // }
		  // std::cout<<std::endl;
		  // 
		  // read legendre polynomial for axial distribution 
		  if (check_for_node(node_nuc, "axial_coeffs")) {
			...
			// assign the flag to indicate axial distribution using legendre polynomial
...
void Material::calculate_neutron_xs(Particle& p) const
{
  ...
  for (int i = 0; i < nuclide_.size(); ++i) {
	...
	if (continuous_num_density_) {
	  atom_density = poly_densities_[i].evaluate(p.coord_[p.n_coord_-1].r);
	  //std::cout<<p.coord_[p.n_coord_-1].r<<'\t'<<p.coord_[p.n_coord_-1].r<<std::endl;
	  //std::cout<<i_nuclide<<'\t'<<atom_density<<"\t micros="<<micro.total<<std::endl;
	  //if (atom_density <= 0.0) {
	  //  //std::cout<<data::nuclides[i_nuclide]->name_<<'\t'<<atom_density<<'\t'<<atom_density_(i)<<std::endl;
	  //  atom_density = atom_density_(i);
	  //}
    } else {
	  atom_density = atom_density_(i);
    }
	//std::cout<<atom_density<<std::endl;
...
void Material::set_fet(const std::string& fet_type, int order, double radius)
{
  ...
  for (gsl::index i = 0; i < nuclide_.size(); ++i) {
    ...
    //poly_densities_[i].center_offset_[0] = 0.0;  // offset.x
    //poly_densities_[i].center_offset_[1] = 0.0;  // offset.y
	//poly_densities_[i].min_ = -1.0;
	//poly_densities_[i].max_ = 1.0;
    
  }
  continuous_num_density_ = true;
}

void Material::set_offsets_fet(const std::vector<double>& x, 
  ...
  const std::vector<double>& density_fet)
{
  ...
  for (gsl::index i = 0; i < n; ++i) {
    ...
    for (int j = 0; j < n_coeffs; j++)
    {
      ...
      //std::cout<<poly_densities_[i].coeffs_[j]<<"\t";
    }
	//std::cout<<std::endl;
  }
  //
...
void Material::set_all_fet(const std::vector<std::string>& names, 
...
extern "C" int 
openmc_material_set_offsets_fet(int32_t index, int n, 
...
extern "C" int
openmc_material_set_all_fet(int32_t index, int n, int m, 
                            const char** names, 
							...
							const double* coeffs)
{
  if (index >= 0 && index < model::materials.size()) {
    try {
	  model::materials[index]->set_all_fet({names, names + n},
...
// cvmt functions 
double 
PolyProperty::evaluate(Position r) const {
  double results {0.0}; 
  // 2D mode 
  if(type_ == "zernike1d"){ 
...
double 
PolyProperty::evaluate_zernike1d(Position r) const {
  int i, k;
  double rho {0.0};
  ...
  //
  if (property < -1.0E-8) {
	//std::cout<<"r.x="<<r.x<<" r.y="<<r.x<<" offset.x="<<center_offset_[0]<<" offset.y="<<center_offset_[1]<<std::endl;
	//fatal_error("A negative number density below -1E-8 was calculated in zernike1d.\n");
...
double 
PolyProperty::evaluate_zernike(Position r) const {
  int i, j, k;
  double rho {0.0};
  double phi {0.0};
  double property {0.0}; 
  ...
  if (property < -1.0E-8) {
    //std::cout<<"r.x="<<r.x<<" r.y="<<r.x<<" offset.x="<<center_offset_[0]<<" offset.y="<<center_offset_[1]<<std::endl;
	//fatal_error("A negative number density below -1E-8 was calculated in zernike.\n");
	property = coeffs_[0]; // negative value is forced to average value 
	//property = 0.0;
  } else if (property < 0.0) {
...
double 
PolyProperty::evaluate_legendre(Position r) const{
  double rho {0.0};
  double property {0.0}; 
  std::vector<double> poly_results;
  if (order_ == 0) return coeffs_[0];
  ...
  //std::cout<<rho<<"\t"<<order_<<"\t"<<n_coeffs_<<"\t"<<property<<"\t"<<coeffs_[0]<<std::endl;  
  if (property < -1.0E-8) {
	//std::cout<<"r.x="<<r.x<<" r.y="<<r.x<<" offset.x="<<center_offset_[0]<<" offset.y="<<center_offset_[1]<<std::endl;
    //fatal_error("A negative number density below -1E-8 was calculated in legendre.\n");
	property = coeffs_[0]; // negative value is forced to average value 
	//property = 0.0;
  } else if (property < 0.0) {
    //
...
double 
PolyProperty::evaluate_legendre2d(Position r) const{
  ...
  // if (rho1 >= -1.0 && rho1 <=1.0 && rho2 >= -1.0 && rho2 <= 1.0) { 
  // }  else { 
  // res1 = coeffs_[0];
  // res2 = coeffs_[n_coeffs_/2]; 
  // }
  // calculate polynomial terms 
...
double 
PolyProperty::evaluate_legendre_axial(Position r) const{
  double rho {0.0};
  double property {0.0}; 
  std::vector<double> poly_results;
  //! allocate array
...
void 
PolyProperty::set_order(std::string type, int order){
...
void 
PolyProperty::set_coeffs(double coeffs[]){
...
void 
PolyProperty::apply_normalization(){
  if(type_ == "zernike1d") {
    int ii = 1;
...
void 
PolyProperty::set_order_axial(int order){
  order_axial_ = order;
...
void 
PolyProperty::set_coeffs_axial(double coeffs_axial[]){
  for (int i=0; i<n_coeffs_axial_; i++){
    coeffs_axial_[i] = coeffs_axial[i];
	//std::cout<<coeffs_axial_[i] <<"\t";
  }
  //std::cout<<std::endl;
}

void 
PolyProperty::set_axial_offset(double zmin, double zmax){

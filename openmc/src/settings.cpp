...
int cvmt_intervals {10};                //! cvmt: number of intervals in cvmt sampling 
} // namespace settings
...
void read_settings_xml()
{
  ...
  // check number of samplings in cvmt 
  if (check_for_node(root, "cvmt_intervals")) {
    cvmt_intervals = std::stoi(get_node_value(root, "cvmt_intervals"));
    if (!run_CE) {
      fatal_error("CVMT transport is not currently supported in "
        "multigroup mode");
    }
  }
}

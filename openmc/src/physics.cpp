...
int sample_nuclide(const Particle* p)
{
  ...
  for (int i = 0; i < n; ++i) {
	...
	if (mat->continuous_num_density_) {
	  atom_density = mat->poly_densities_[i].evaluate(p->coord_[p->n_coord_-1].r);
	  //std::cout<<p->coord_[p->n_coord_-1].r<<std::endl;
	  //std::cout<<i_nuclide<<'\t'<<atom_density<<std::endl;
    } else {
